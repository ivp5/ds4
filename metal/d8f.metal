...
static float4 d8f_mid_value4(device const uchar *pack, ulong scale_off, device const float *slot_mid, uint idx) {
  const device float4 *slot_mid4 = (const device float4 *)(slot_mid + idx);
  float4 v = *slot_mid4;
  if (scale_off == 0ul) return v;
  const device half4 *inv_scale4 = (const device half4 *)(pack + scale_off + ulong(idx) * 2ul);
  return v * float4(*inv_scale4);
}

static float d8f_dot_codebook8(device const half *cb, float4 m0, float4 m1) {
  const device half4 *cb4 = (const device half4 *)cb;
  return dot(float4(cb4[0]), m0) + dot(float4(cb4[1]), m1);
}
...
struct D8FRecordLite {
  ...
  uint scale_bytes;
  uint row_base;
  ulong native_code_offset;
  uint native_code_bytes;
  uint native_code_flags;
};
...
struct D8FDownLutI8CodebookLite {
  uint k;
  uint offset;
  uint scale_offset;
  uint reserved;
...
kernel void d8f_down_sparse_score_indirect_prepare(
  device const D8FSparseDownRecordLite *sparse  [[buffer(0)]],
  device const uint *selected                   [[buffer(1)]],
  device uint *slot_prefix                      [[buffer(2)]],
  device uint *indirect_args                    [[buffer(3)]],
  constant D8FSparseDownArgs &args              [[buffer(4)]],
  uint tid [[thread_position_in_grid]]) {
  if (tid != 0u) return;
  const uint total_slots = args.n_tokens * args.n_selected;
  uint total_unique = 0u;
  slot_prefix[0] = 0u;
  for (uint token_slot = 0u; token_slot < total_slots; token_slot++) {
    const uint selected_denom = args.n_selected == 0u ? 1u : args.n_selected;
    const uint token = token_slot / selected_denom;
    const uint slot = token_slot - token * args.n_selected;
    uint add = 0u;
    if (token < args.n_tokens && slot < args.n_selected) {
      const uint expert = selected[ulong(token) * ulong(args.selected_token_stride) + ulong(slot)];
      const D8FSparseDownRecordLite srec = sparse[expert];
      if ((srec.flags & 1u) != 0u) add = srec.unique_count;
    }
    total_unique += add;
    slot_prefix[token_slot + 1u] = total_unique;
  }
  indirect_args[0] = (total_unique + 255u) >> 8;
  indirect_args[1] = 1u;
  indirect_args[2] = 1u;
}

kernel void d8f_down_sparse_score_selected_compact_indirect(
  device const uchar *sparse_pack               [[buffer(0)]],
  device const uchar *pack                      [[buffer(1)]],
  device const D8FSparseDownRecordLite *sparse  [[buffer(2)]],
  device const D8FRecordLite *recs              [[buffer(3)]],
  ...
  device float *score                           [[buffer(6)]],
  constant D8FSparseDownArgs &args              [[buffer(7)]],
  device const uint *slot_prefix                [[buffer(8)]],
  device const ushort *unique_group             [[buffer(9)]],
  uint tid [[thread_index_in_threadgroup]],
  uint3 tgp [[threadgroup_position_in_grid]]) {
  const uint linear = tgp.x * 256u + tid;
  const uint total_slots = args.n_tokens * args.n_selected;
  const uint total_unique = slot_prefix[total_slots];
  if (linear >= total_unique) return;
  uint lo_slot = 0u;
  uint hi_slot = total_slots;
  while (lo_slot + 1u < hi_slot) {
    const uint mid_slot_idx = (lo_slot + hi_slot) >> 1;
    if (slot_prefix[mid_slot_idx] <= linear) {
      lo_slot = mid_slot_idx;
    } else {
      hi_slot = mid_slot_idx;
    }
  }
  const uint token_slot = lo_slot;
  const uint unique_index = linear - slot_prefix[token_slot];
  const uint selected_denom = args.n_selected == 0u ? 1u : args.n_selected;
  const uint token = token_slot / selected_denom;
  const uint slot = token_slot - token * args.n_selected;
  if (token >= args.n_tokens || slot >= args.n_selected) return;
  const uint expert = selected[ulong(token) * ulong(args.selected_token_stride) + ulong(slot)];
  const D8FSparseDownRecordLite srec = sparse[expert];
  if ((srec.flags & 1u) == 0u || unique_index >= srec.unique_count) return;
  device const uint *prefix = (device const uint *)(sparse_pack + srec.group_prefix_offset);
  const uint group = uint(unique_group[ulong(srec.reserved0) + ulong(unique_index)]);
  const uint start = prefix[group];
  const uint end = prefix[group + 1u];
  if (end <= start || end > srec.unique_count || unique_index < start || group >= (args.in_dim >> 3)) return;
  const uint local = unique_index - start;
  if (local >= args.max_group_unique) return;
  device const ushort *unique_codes = (device const ushort *)(sparse_pack + srec.unique_code_offset);
  ...
  const float m6 = d8f_mid_value(pack, rec.scale_offset, slot_mid, x_base + 6u);
  const float m7 = d8f_mid_value(pack, rec.scale_offset, slot_mid, x_base + 7u);
  score[(ulong(token_slot) * ulong(args.in_dim >> 3) + ulong(group)) *
        ulong(args.max_group_unique) + ulong(local)] =
      d8f_dot8_codebook_buffer(pack, rec.codebook_offset, code, m0, m1, m2, m3, m4, m5, m6, m7);
}
...
inline uint2 d8f_pack2d_coord(uint texel) {
  constexpr uint width = 16384u;
  const uint y = texel / width;
  return uint2(texel - y * width, y);
}

inline float d8f_dot8_codebook_pack2d(texture2d<half, access::read> codebook_tex,
                                      ulong codebook_offset,
                                      uint code,
                                      float m0,
                                      float m1,
                                      ...
                                      float m7) {
  const uint base = uint((codebook_offset >> 3) + ulong(code) * 2ul);
  const half4 lo = codebook_tex.read(d8f_pack2d_coord(base));
  const half4 hi = codebook_tex.read(d8f_pack2d_coord(base + 1u));
  return float(lo.x) * m0 + float(lo.y) * m1 +
         float(lo.z) * m2 + float(lo.w) * m3 +
         float(hi.x) * m4 + float(hi.y) * m5 +
         float(hi.z) * m6 + float(hi.w) * m7;
...
kernel void d8f_down_lut_score_vec_selected_batch(
  device const uchar *pack          [[buffer(0)]],
  device const float *mid           [[buffer(1)]],
  device const uint  *selected      [[buffer(2)]],
  device float       *score         [[buffer(3)]],
  ...
  const device float *slot_mid =
      ...
      ulong(slot) * ulong(args.mid_slot_stride);
  const device half *cb = (const device half *)(pack + rec.codebook_offset + ulong(code) * 16ul);
  const float4 m0 = d8f_mid_value4(pack, rec.scale_offset, slot_mid, x_base);
  const float4 m1 = d8f_mid_value4(pack, rec.scale_offset, slot_mid, x_base + 4u);
  score[out_index] = d8f_dot_codebook8(cb, m0, m1);
}

kernel void d8f_down_lut_score_i8_selected_batch(
  device const uchar *pack          [[buffer(0)]],
  device const float *mid           [[buffer(1)]],
  device const uint  *selected      [[buffer(2)]],
  device float       *score         [[buffer(3)]],
  constant D8FDownLutArgs &args     [[buffer(4)]],
  device const D8FRecordLite *recs  [[buffer(5)]],
  device const D8FDownLutI8CodebookLite *i8_recs [[buffer(6)]],
  device const char *i8_codebook    [[buffer(7)]],
  uint3 gid [[thread_position_in_grid]]) {
  const uint code = gid.x;
  const uint group = gid.y;
  const uint token_slot = gid.z;
...
kernel void d8f_down_lut_gather_codes_selected_batch_tile32(
  device const ushort *codes        [[buffer(0)]],
  device const float  *score        [[buffer(1)]],
  device float        *out          [[buffer(2)]],
  constant D8FDownLutArgs &args     [[buffer(3)]],
...
kernel void d8f_down_lut_direct_codes_selected_batch_tile8(
  device const ushort *codes        [[buffer(0)]],
  device const uchar  *pack         [[buffer(1)]],
...
kernel void d8f_down_lut_direct_codes_selected_batch_tile16(
  device const ushort *codes        [[buffer(0)]],
  device const uchar  *pack         [[buffer(1)]],
  device const float  *mid          [[buffer(2)]],
  device const uint   *selected     [[buffer(3)]],
...
kernel void d8f_down_lut_direct_codes_selected_batch_tile16_cbsram1024(
  device const ushort *codes        [[buffer(0)]],
  device const uchar  *pack         [[buffer(1)]],
  device const float  *mid          [[buffer(2)]],
  device const uint   *selected     [[buffer(3)]],
  ...
  device const float *sidecar_dot   [[buffer(8)]],
  threadgroup float *partial        [[threadgroup(0)]],
  threadgroup half *cb_cache        [[threadgroup(1)]],
  uint tid [[thread_index_in_threadgroup]],
  ushort tiisg [[thread_index_in_simdgroup]],
  ushort sgitg [[simdgroup_index_in_threadgroup]],
  uint2 pos [[threadgroup_position_in_grid]]) {
  ...
  for (uint slot = 0u; slot < args.n_selected; slot++) {
    ...
    const device float *slot_mid =
        mid + ulong(token) * ulong(args.mid_token_stride) +
        ulong(slot) * ulong(args.mid_slot_stride);
    const bool use_cb_cache = cache_k_cap > 0u && rec.k > 0u && rec.k <= cache_k_cap;
    if (use_cb_cache) {
      for (uint chunk_base = 0u; chunk_base < rec.k; chunk_base += 1024u) {
        const uint chunk_k = min(1024u, rec.k - chunk_base);
        device const half *cb_src = (const device half *)(pack + rec.codebook_offset + ulong(chunk_base) * 16ul);
        const uint cb_vals = chunk_k << 3;
        ...
        for (uint group = tid; group < groups; group += 256u) {
          ...
          for (uint rr = 0u; rr < 16u; rr++) {
            ...
            if (row >= args.rows) continue;
            const uint code = uint(codes[(ulong(slot) * ulong(args.rows) + ulong(row)) * ulong(groups) + ulong(group)]);
            if (code < chunk_base || code >= chunk_base + chunk_k) continue;
            threadgroup const half *cb = cb_cache + ulong(code - chunk_base) * 8ul;
            acc[rr] +=
                float(cb[0]) * m0 +
                float(cb[1]) * m1 +
                float(cb[2]) * m2 +
...
kernel void d8f_down_sum_selected_weighted_batch_tile16_recbuf_native_codes(
  device const uchar *pack               [[buffer(0)]],
  device const float *mid                [[buffer(1)]],
  device const uint  *selected           [[buffer(2)]],
  device float       *out                [[buffer(3)]],
...
kernel void d8f_down_sum_selected_weighted_batch_tile16_recbuf_native_codes_i8_cbsram(
  device const uchar *pack               [[buffer(0)]],
  device const float *mid                [[buffer(1)]],
  device const uint  *selected           [[buffer(2)]],
  device float       *out                [[buffer(3)]],
  ...
  device const D8FRecordLite *recs       [[buffer(6)]],
  device const float *sidecar_dot        [[buffer(7)]],
  device const D8FDownLutI8CodebookLite *i8_recs [[buffer(8)]],
  ...
  for (uint slot = 0u; slot < args.n_selected; slot++) {
    ...
    const uint expert = selected[ulong(token) * ulong(args.selected_token_stride) + ulong(slot)];
    const D8FRecordLite rec = recs[8192u + expert];
    const D8FDownLutI8CodebookLite i8 = i8_recs[expert];
    device const uchar *side_rec = d8f_down_sidecar_record(pack, args, expert);
    const uint side_rank = side_rec ? d8f_u32(side_rec + 4) : 0u;
    const uint side_in_dim = side_rec ? d8f_u32(side_rec + 8) : 0u;
    const uint side_out_dim = side_rec ? d8f_u32(side_rec + 12) : 0u;
...
kernel void d8f_down_sum_selected_weighted_batch_tile16_recbuf_native_codes_pack2d(
  device const uchar *pack               [[buffer(0)]],
  device const float *mid                [[buffer(1)]],
  device const uint  *selected           [[buffer(2)]],
  device float       *out                [[buffer(3)]],
  ...
  for (uint slot = 0u; slot < args.n_selected; slot++) {
    ...
    for (uint block_col = tid; block_col < blocks_per_row; block_col += 256u) {
      ...
      for (uint rr = 0u; rr < 16u; rr++) {
        ...
        if (code >= rec.k) continue;
        if (native_codes) {
          acc[rr] += rw * d8f_dot8_codebook_pack2d(codebook_tex, rec.codebook_offset, code,
                                                     m0, m1, m2, m3, m4, m5, m6, m7);
        } else {
          const device half *cb = (const device half *)(pack + rec.codebook_offset + ulong(code) * 16ul);
          acc[rr] += rw * (float(cb[0]) * m0 + float(cb[1]) * m1 +
