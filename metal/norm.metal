...
struct ds4_metal_args_rms_norm_rope_tail {
    int32_t head_dim;
    int32_t n_rot;
    int32_t pos;
    int32_t n_ctx_orig;
...
static float norm_rope_yarn_ramp(const float low, const float high, const int i0) {
    const float y = (i0 / 2 - low) / max(0.001f, high - low);
    return 1.0f - min(1.0f, max(0.0f, y));
}

static float norm_rope_yarn_corr_factor(int n_dims, int n_ctx_orig, float n_rot, float base) {
    return n_dims * log(n_ctx_orig / (n_rot * 2 * M_PI_F)) / (2 * log(base));
}

static void norm_rope_yarn_corr_dims(int n_dims, int n_ctx_orig, float freq_base,
                                     float beta_fast, float beta_slow, thread float dims[2]) {
    dims[0] = max(0.0f, floor(norm_rope_yarn_corr_factor(n_dims, n_ctx_orig, beta_fast, freq_base)));
    dims[1] = min(n_dims - 1.0f, ceil(norm_rope_yarn_corr_factor(n_dims, n_ctx_orig, beta_slow, freq_base)));
}

static void norm_rope_yarn(float theta_extrap, float freq_scale, float corr_dims[2],
                           int i0, float ext_factor, float mscale,
                           thread float *cos_theta, thread float *sin_theta) {
    float theta_interp = freq_scale * theta_extrap;
    float theta = theta_interp;
    if (ext_factor != 0.0f) {
        float ramp_mix = norm_rope_yarn_ramp(corr_dims[0], corr_dims[1], i0) * ext_factor;
...
kernel void kernel_dsv4_rms_norm_weight_rope_tail_f32(
        constant ds4_metal_args_rms_norm_rope_tail & args,
    ...
    if (args.head_dim <= 0 || args.n_rot < 0 || args.n_rot > args.head_dim || (args.n_rot & 1) != 0) {
        return;
    }

    if (sg == 0) scratch[lane] = 0.0f;

    float sumf = 0.0f;
    for (int i = tid; i < args.head_dim; i += ntg) {
        const float v = x[i];
        sumf += v * v;
    }
    sumf = simd_sum(sumf);
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (lane == 0) scratch[sg] = sumf;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    sumf = scratch[lane];
    sumf = simd_sum(sumf);
    const float scale = 1.0f / sqrt(sumf / float(args.head_dim) + args.eps);

    const int n_nope = args.head_dim - args.n_rot;
    for (int i = tid; i < n_nope; i += ntg) {
        out[i] = x[i] * scale * weight[i];
    }
    if (args.n_rot == 0) return;

    float corr_dims[2];
    norm_rope_yarn_corr_dims(args.n_rot, args.n_ctx_orig, args.freq_base,
                             args.beta_fast, args.beta_slow, corr_dims);
    const float theta_base = float(args.pos);
    const float inv_ndims = -1.0f / float(args.n_rot);
    const int n_pair = args.n_rot >> 1;
    for (int pair = tid; pair < n_pair; pair += ntg) {
        const int r = pair << 1;
        ...
        float cos_theta;
        float sin_theta;
        norm_rope_yarn(theta, args.freq_scale, corr_dims, r,
                       args.ext_factor, args.attn_factor, &cos_theta, &sin_theta);
        const int j0 = n_nope + r;
        const int j1 = j0 + 1;
        const float x0 = x[j0] * scale * weight[j0];
        const float x1 = x[j1] * scale * weight[j1];
