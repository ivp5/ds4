...
static void metal_graph_read_decode_policy_from_environment_once(void) {
 ...
 g_decode_policy.uses_reference_hc = ds4_env_enabled("DS4_METAL_DISABLE_HC_FUSION");
 g_decode_policy.uses_reference_kv = ds4_env_enabled("DS4_METAL_DISABLE_KV_FUSION");
 g_decode_policy.uses_reference_qkv_norm = ds4_env_enabled("DS4_METAL_DISABLE_QKV_NORM_FUSION");
 g_decode_policy.uses_reference_compressor_pair_proj = ds4_env_enabled("DS4_METAL_DISABLE_COMPRESSOR_PAIR_PROJ");
 g_decode_policy.uses_reference_hc_norm =
 !ds4_env_enabled("DS4_METAL_ENABLE_HC_NORM_FUSION") ||
 ds4_env_enabled("DS4_METAL_DISABLE_HC_NORM_FUSION");
 g_decode_policy.uses_reference_shared_down_hc = ds4_env_enabled("DS4_METAL_DISABLE_SHARED_DOWN_HC_FUSION");
 g_decode_policy.uses_reference_attn_out_hc = ds4_env_enabled("DS4_METAL_DISABLE_ATTN_OUT_HC_FUSION");
 g_decode_policy.uses_kv_rope_store_fusion = !ds4_env_enabled("DS4_METAL_DISABLE_KV_ROPE_STORE_FUSION");
 g_decode_policy.uses_q_head_norm_rope_fusion =
 (max_fusion || ds4_env_enabled("DS4_METAL_ENABLE_Q_HEAD_NORM_ROPE_FUSION")) &&
 !ds4_env_enabled("DS4_METAL_DISABLE_Q_HEAD_NORM_ROPE_FUSION");
 g_decode_policy.uses_indexer_q_rope_fusion =
 (max_fusion || ds4_env_enabled("DS4_METAL_ENABLE_INDEXER_Q_ROPE_FUSION")) &&
 !ds4_env_enabled("DS4_METAL_DISABLE_INDEXER_Q_ROPE_FUSION");
 g_decode_policy.uses_indexed_attn_rope_fusion =
 ds4_env_enabled("DS4_METAL_ENABLE_INDEXED_ATTN_ROPE_FUSION") &&
 !ds4_env_enabled("DS4_METAL_DISABLE_INDEXED_ATTN_ROPE_FUSION");
 g_decode_policy.uses_decode_attn_rope_fusion =
 ds4_env_enabled("DS4_METAL_ENABLE_DECODE_ATTN_ROPE_FUSION") &&
 !ds4_env_enabled("DS4_METAL_DISABLE_DECODE_ATTN_ROPE_FUSION");
 g_decode_policy.uses_router_matmul_select_fusion =
 (max_fusion || ds4_env_enabled("DS4_METAL_ENABLE_ROUTER_MATMUL_SELECT_FUSION")) &&
 !ds4_env_enabled("DS4_METAL_DISABLE_ROUTER_MATMUL_SELECT_FUSION");
 g_decode_policy.uses_fp8_attn_out_onecb_hc =
 ...
 g_decode_policy.uses_hc_rms_mix_fusion = !ds4_env_enabled("DS4_METAL_DISABLE_HC_RMS_MIX_FUSION");
...
static bool metal_graph_decode_hc_pre(
 ...
 if (g_decode_policy.uses_reference_hc) {
...
static bool metal_graph_decode_kv_store(
 ...
 uint32_t raw_row) {
 metal_graph_read_decode_policy_from_environment_once();
 if (g_decode_policy.uses_reference_kv) {
...
static int ds4_hc_rms_f16_mix_via_tensor(ds4_gpu_tensor *dst,
                                          ...
                                          float eps) {
 metal_graph_read_decode_policy_from_environment_once();
 if (!g_decode_policy.uses_hc_rms_mix_fusion ||
     !dst || !model || !t || !src ||
     t->type != DS4_TENSOR_F16 ||
...
static int ds4_hc_full_prelude_f16_via_tensor(ds4_gpu_tensor *out,
                                              ...
                                              float norm_eps) {
 metal_graph_read_decode_policy_from_environment_once();
 if (!g_decode_policy.uses_hc_full_prelude_fusion ||
     !g_decode_policy.uses_hc_rms_mix_fusion ||
...
static int ds4_output_hc_full_f16_via_tensor(ds4_gpu_tensor *pre_out,
                                             ...
                                             uint32_t n_embd,
                                              ...
                                              float norm_eps) {
 metal_graph_read_decode_policy_from_environment_once();
 if (!g_decode_policy.uses_output_hc_full_fusion ||
     !g_decode_policy.uses_hc_rms_mix_fusion ||
     !g_decode_policy.uses_output_hc_sum_norm_fusion ||
...
static bool metal_graph_encode_decode_layer(
 ...
 metal_graph_read_decode_policy_from_environment_once();
 const bool qkv_rms_fused = !g_decode_policy.uses_reference_qkv_norm;
...
#define DS4_METAL_PROFILE_DECODE_STAGE(name) do { \
 ...
 !g_decode_policy.uses_reference_hc &&
 !g_decode_policy.uses_reference_hc_norm;
 ...
 if (ok && g_decode_policy.uses_q_head_norm_rope_fusion) {
 ...
 if (ok &&
 g_decode_policy.uses_kv_rope_store_fusion &&
 !g_decode_policy.uses_reference_kv &&
 ...
 if (ok && !g_decode_policy.uses_reference_compressor_pair_proj) {
 ...
 if (ok && g_decode_policy.uses_indexer_q_rope_fusion) {
 ...
 * avoiding the long-context decode failure. */
 ...
 if (g_decode_policy.uses_indexed_attn_rope_fusion &&
 ...
 if (g_decode_policy.uses_decode_attn_rope_fusion &&
 !metal_graph_debug_wants("kqv_out", il, pos)) {
 ...
 layer->attn_output_a->storage.scale_dtype == DS4_TENSOR_FP8_E8M0 &&
  ...
  if (fuse_fp8_attn_out_hc && g_decode_policy.uses_fp8_attn_out_onecb_hc) {
 ...
 !keep_ffn_out && shared_down_q8_native && !g_decode_policy.uses_reference_shared_down_hc;
 const bool fuse_shared_down_fp8_hc =
 !keep_ffn_out &&
 g_decode_policy.uses_fp8_shared_down_hc &&
 shared_down_fp8_storage &&
 !metal_graph_directional_steering_ffn_enabled(g) &&
 !g_decode_policy.uses_reference_shared_down_hc;
...
#define DS4_METAL_PROFILE_OUTPUT_STAGE(name) do { \
 ...
 metal_graph_read_decode_policy_from_environment_once();
 if (ok && !output_hc_full_fused && g_decode_policy.uses_output_hc_sum_norm_fusion) {
