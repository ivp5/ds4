#!/usr/bin/env python3
"""Wire remaining F16 production call sites through ds4_matmul_f16_via_tensor.

Pattern transform:
  ds4_gpu_matmul_f16_tensor(DST, MODEL_ARGS, X->abs_offset, IN, OUT, SRC, N)
  →
  ds4_matmul_f16_via_tensor(DST, model, X, IN, OUT, SRC, N)
...
# Find every "ds4_gpu_matmul_f16_tensor(" call; expand to closing ")"
calls = []
i = 0
while i < len(lines):
    if "ds4_gpu_matmul_f16_tensor(" in lines[i]:
        ...
        for j in range(start, min(start + 25, len(lines))):
            ...
            if depth <= 0 and j > start:
                ...
                break
            elif depth <= 0 and lines[j].count(")") > 0 and "ds4_gpu_matmul_f16_tensor(" in lines[j]:
                # single-line call
                end = j
                break
...
print(f"Found {len(calls)} ds4_gpu_matmul_f16_tensor( call sites")

# For each call, parse args. Skip if this is the dispatcher's fallback.
def is_dispatcher_fallback(chunk):
    """The dispatcher's own fallback is `return ds4_gpu_matmul_f16_tensor(...)`."""
    full = "\n".join(chunk)
    return "return ds4_gpu_matmul_f16_tensor(dst, model->map, model->size," in full

def parse_call(chunk):
    """Extract (dst, weight_pointer_expr, in_dim, out_dim, src, n_tok)."""
    full = "\n".join(chunk)
    # Find the "ds4_gpu_matmul_f16_tensor(" position and walk forward
    ...
    start = full.find("ds4_gpu_matmul_f16_tensor(")
    if start < 0:
        return None
    after = start + len("ds4_gpu_matmul_f16_tensor(")
    depth = 1
    i = after
...
def fmt_call(parts, leading_ws):
    ...
    return (
        f"{leading_ws}ds4_matmul_f16_via_tensor({parts['dst']}, model,\n"
...
if not APPLY:
    for (start, end, chunk, parts, model_var) in plans[:3]:
        ...
        prefix_match = re.search(r"^(.*?)ds4_gpu_matmul_f16_tensor", chunk[0])
        prefix = prefix_match.group(1) if prefix_match else leading_ws
        new_first = f"{prefix}ds4_matmul_f16_via_tensor({parts['dst']}, {model_var},"
        new_body = (
            f"{leading_ws} {parts['tensor_ptr']},\n"
            f"{leading_ws} {parts['in_dim']}, {parts['out_dim']},\n"
            f"{leading_ws} {parts['src']}, {parts['n_tok']})"
...
for (start, end, chunk, parts, model_var) in sorted(plans, key=lambda p: -p[0]):
    ...
    suffix = last[close_idx + 1:] if close_idx >= 0 else ""
    new_call = (
        f"{prefix}ds4_matmul_f16_via_tensor({parts['dst']}, {model_var},\n"
        f"{leading_ws} {parts['tensor_ptr']},\n"
        f"{leading_ws} {parts['in_dim']}, {parts['out_dim']},\n"
