...
## 2026-06-04T01:23 JST — low-level MPSGraph/CoreML loophole pass

- Fixed `metal_vqd8_real_d8f_down_probe.m` selected-expert parsing and widened the real packed Metal probe to mixed per-expert `k/bits/index_offset` metadata. The real selected-six down set is heterogeneous: `165/k4096b12,0/k2048b11,1/k1024b10,2/k4096b12,3/k1024b10,4/k4096b12`.
- Mixed selected-six packed Metal is correct: `3081.148 us/op`, `bad=0`, `rms=0.000282924`, `index_MB=8.782`, `codebook_MB=0.393`. It is not yet faster than MPSGraph selected-six, but it is self-contained and no longer assumes uniform VQ width.
- SDK 26.5 MPSGraph headers expose only `MPSGraphDeviceTypeMetal`, but compile optimization level 1 explicitly claims placement across NeuralEngine/CPU/GPU; practical evidence remains noisy. Added `DS4_MPSGRAPH_COMPILE_MODE={default,level0,level1,fastmath,runtime_type_infer}` as a real canary gate, not a default.
- Added `DS4_MPSGRAPH_ASYNC_BATCH=1` timing mode using `MPSGraphExecutableExecutionDescriptor` plus `MTLSharedEvent` final wait. Synthetic 12-bit VQ-D8 MPSGraph r50 improved expanded sync `735.787 us` to async-batch `398.767 us`; packed sync `1421.571 us` to async-batch `695.074 us`.
- Real H3355 selected-six MPSGraph async-batch passed correctness and improved same-window r50: down sync `1609.519 us` -> async-batch `1081.396 us`; gate/up sync `1719.212 us` -> async-batch `1242.824 us`; both `bad=0`.
- CoreML compute-plan probe now uses fast-prediction hints and reports preferred devices. The local shared L0 b2048 8-bit package has `ops=21`, `ane_preferred=9`, `ane_supported=9`, `unknown_usage=12`; ANE is a real placement target for matmul/lut-dense/silu/min/max, while constants/identity remain unplanned or unknown.
- Decision: the concrete low-level MPSGraph win is not packed decode itself; it is queueing multiple executable runs asynchronously and waiting once. This matters for prefill/layer pipelines and ANE overlap, not single isolated token decode unless adjacent work can be batched or overlapped.

## 2026-06-04T01:34 JST — undocumented API permutation tests

- Added `private_api_surface_probe.m` to enumerate runtime selectors/properties for MPSGraph/CoreML classes. Important private runtime surface exists: `MPSGraphDevice.ANEDevice`, MPSGraph descriptor fields for `preferredDevice`, `allowedComputeDevices`, ANE flags, executable execution flags, CoreML `e5rtComputeDeviceTypeMask`, `experimentalMLE5EngineUsage`, and `MLPredictionOptions.aneQoS/aneExecutionPriority`.
- MPSGraph private compile permutations all executed without process crashes, including `private_ane_device`. None produced a stable production-worthy speedup over the public async-batch path. `private_prefer2` was clearly harmful on synthetic 12-bit VQ-D8: expanded sync rose to `1576.581 us/op`, packed sync to `2360.758 us/op`.
- MPSGraph private execution flags (`disableSynchronizeResults`, `disableANECaching`, `disableANEFallback`, `encodeANESync`, `encodeANEDisableSharedEvents`) did not beat the already useful public-ish async/shared-event batching path in a stable way.
- CoreML private config modes exposed useful placement control. `e5rtComputeDeviceTypeMask=1` and `=3` forced CPU-preferred placement (`ane_preferred=0`, `cpu_preferred=9`) and cut load time to about `96-102 ms`, but steady prediction slowed to about `26.8 ms` versus default ANE steady `12.2-13.9 ms`.
- `experimentalMLE5EngineUsage=1` also cut load to about `382 ms`, but increased footprint to about `345 MiB` and slowed steady prediction to `27.68 ms`; reject for hot inference unless load latency alone dominates.
- `e5rtComputeDeviceTypeMask=7`, `experimentalMLE5EngineUsage=2`, `specializationUsesMPSGraphExecutable`, GPU accumulation flags, background GPU, and instrumentation preserved ANE placement and were roughly default-like in this package.
- `MLPredictionOptions.aneQoS` accepted user/utility values but did not improve steady prediction in the isolated package. `aneExecutionPriority` strings `high`, `low`, and `realtime` are invalid on this runtime and abort the process with `Unrecognized ANE execution priority`; do not use unvalidated string priorities.
- Decision: keep private API usage inside probes/env gates. The only private knob with architectural value is `e5rtComputeDeviceTypeMask` as a placement falsifier; it proves the ANE speedup is real by forcing CPU fallback, but it is not the desired runtime mode.
